...
- `2017-05-12_batchdata_updated_struct_errorcorrect.mat` -> `batch1.pkl` -> `features_top8_cycles.csv` (138 rows, 46 cells x 3 early-cycle windows).
...
- Each model trains with an 80/20 split inside every `n_cycles in {25, 50, 100}` slice. Metrics are stored in `results_top8_metrics.json` and summarized below.
...
- Use `python 2_modeling_featuring/split_train_val_test.py` to generate deterministic CSVs for each split. By default the script shuffles `cell_id` values with seed 42 and writes `features_top8_cycles_{train,val,test}.csv`.
...
- Running `python 2_modeling_featuring/cross_validate_models.py` produces `results_top8_cv_metrics.json`, which stores the mean ± std of MAE, R², MAPE, and SMAPE for each model / feature set / `n_cycles` slice.
...
- The script saves `results_catboost_cv_selected.json` and a companion table `plots/table_catboost_cv_selected.png` summarizing the chosen hyperparameters, CV MAE (mean ± std), and final test MAE/R²/MAPE/SMAPE for both feature sets (with vs without Qd_std).
...
- `results_top8_metrics.json`: Hold-out MAE/R²/MAPE for RandomForest, XGBoost, CatBoost (with/without Qd_std, per window).
- `results_top8_cv_metrics.json`: Grouped 5-fold CV mean ± std for MAE/R²/MAPE/SMAPE (all four models).
- `results_catboost_cv_selected.json`: CV-selected CatBoost hyperparameters plus final train+val→test metrics.
- `results_conformal_<model>.json`: per-model conformal quantiles and coverage values (train on train, calibration on val, evaluated on test).
...
- CatBoost (baseline): iterations=400, learning_rate=0.05, depth=6, loss=MAE. CV-selected variants listed in `results_catboost_cv_selected.json`.
