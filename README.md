...
├── legacy/                  # archived reference/older materials + old outputs
├── run_pipeline.py          # orchestrator (--status, --resume, --phase, --stages)
├── requirements.txt
└── README.md
```

The repository has been cleaned so the active thesis workflow is the SOPv2
pipeline in `0_data/`, `1_features/`, `2_models/`, and `3_analysis/`.
`legacy/` is retained only for traceability and backup; it is not used by
`run_pipeline.py`. Files that existed only in the old `archive/` folder were
moved under `legacy/archive_only/`.
...
python3 run_pipeline.py --status                  # show what's done / missing
python3 run_pipeline.py --phase model             # splits + VIF + within-dataset experiments
python3 run_pipeline.py --phase analysis          # shift + XAI + survival + concept + rescaling + CP
```
...
python3 2_models/vif_screening.py --drop

# Cross-dataset transfer with capacity normalization
python3 1_features/build_features.py --capacity-normalize
python3 2_models/run_experiments.py --cross-dataset --log-target \
    --output-dir outputs/results_v2_cross_34_capnorm

# Distribution shift quantification
python3 3_analysis/shift_metrics.py
python3 3_analysis/shift_metrics.py --capacity-normalize
python3 3_analysis/feature_transfer_stability.py
python3 3_analysis/shap_feature_importance.py
python3 3_analysis/survival_censoring.py

# Concept-shift and dynamics diagnostics
python3 3_analysis/concept_shift_diagnostics.py
python3 3_analysis/koopman_dmd_pilot.py

# Target-mean rescaling
python3 3_analysis/target_rescaling.py
...
python3 3_analysis/conditional_shift_decomposition.py
python3 3_analysis/koopman_dmd_pilot.py
python3 3_analysis/importance_weighted_conformal.py
...
python3 3_analysis/conformal_prediction.py --adapter-types residual_mean linear
```
